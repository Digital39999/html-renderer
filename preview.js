(function () {
	...
	let url = location.search.substring(1).replace(/\/\/github\.com/, '//raw.githubusercontent.com').replace(/\/blob\//, '/');

	let replaceAssets = function () {
		let frame, a, link, links = [], script, scripts = [], i, href, src;
		if (document.querySelectorAll('frameset').length) return;

		frame = document.querySelectorAll('iframe[src],frame[src]'); for (i = 0; i < frame.length; ++i) {
			src = frame[i].src;
		...
		a = document.querySelectorAll('a[href]'); for (i = 0; i < a.length; ++i) {
			href = a[i].href;
		...
		link = document.querySelectorAll('link[rel=stylesheet]'); for (i = 0; i < link.length; ++i) {
			href = link[i].href;
		...
		script = document.querySelectorAll('script[type="text/htmlpreview"]'); for (i = 0; i < script.length; ++i) {
			src = script[i].src;

			if (src.indexOf('//raw.githubusercontent.com') > 0 || src.indexOf('//bitbucket.org') > 0) {
				scripts.push(fetchProxy(src, null, 0));
			} else {
				script[i].removeAttribute('type'); scripts.push(script[i].innerHTML);
			};
		};
	...
	let loadHTML = function (data) {
		if (data) {
			...
			setTimeout(function () {
				document.open(); document.write(data); document.close(); replaceAssets();
			}, 10);
		};
	};

	let loadCSS = function (data) {
		if (data) {
			let style = document.createElement('style');
			style.innerHTML = data; document.head.appendChild(style);
		};
	};
	...
	let loadJS = function (data) {
		if (data) {
			...
			script.innerHTML = data; document.body.appendChild(script);
		};
	};
	
	let fetchProxy = function (url, options, i) {
		let proxy = ['', 'https://api.codetabs.com/v1/proxy/?quest='];
		return fetch(proxy[i] + url, options).then(function (res) {
			if (!res.ok) throw new Error('Cannot load ' + url + ': ' + res.status + ' ' + res.statusText); return res.text();
		}).catch(function (error) {
			if (i === proxy.length - 1) throw error; return fetchProxy(url, options, i + 1);
		});
	};

	if (url && url.indexOf(location.hostname) < 0) {
		fetchProxy(url, null, 0).then(loadHTML).catch(function (error) {
			console.error(error); previewForm.style.display = 'block'; previewForm.innerText = error;
		});
	} else {
